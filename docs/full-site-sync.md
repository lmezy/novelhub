...
2026-09-18 修。如果几百条警告都指向**同一个书源**、而且失败的「章节 URL」其实是**书籍详情页**
（例如御宅屋 yswhub.cc 的 `/read/<书号>.html`，没有第二段章号），说明书源目录规则已经失效：

书源的 `ruleBookInfo.tocUrl` 指向独立目录页 `/indexlist/<书号>/`，该页的章节列表改成了
JavaScript 动态填充（真实浏览器渲染后也是空的），`ruleToc` 于是解析出 0 章。旧代码会**退回扫描
书籍详情页上的所有链接**，而那里只有「相关推荐 / 作者其他作品」——一本书被造出 8 个假章节，
每个都是一本书的详情页，取正文必然为空。

现在：书源**自己声明**的目录页如果解析不出章节，插件直接报
`书源目录规则已失效：书源声明的目录页没有解析出任何章节（<目录页>）…`，任务把这本书记成
**已过滤（类型「目录」）**并继续跑，不再刷章节错误、也不会因为累积失败中止整个任务。
这类书在源站通常真的没有章节，处理办法是换书源或等书源更新；`tocUrl` 是靠页面链接猜出来的
（书源没写规则）时不适用这条，仍会回退扫描书页。
...
- 爱丽丝书屋（alicesw.com）有一个自己的 `访问验证` 页（约 1.5KB，
  `当前访问行为触发了安全验证，请输入验证码后继续阅读`）。2026-09-16 实测：连续同步
  一天（约 1300 多章、已经按 `CRAWL_DELAY_MS=1200` 限速）之后站点开始返回它，
  于是任务报「同步被网站反爬拦截…已连续 5 章失败」。这是**真实拦截**，不是误判；
  隔一段时间再同步，或按上面的办法过验证后导入 Cookie。
