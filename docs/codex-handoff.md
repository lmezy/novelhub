...
## 46. 合集帖「一章=一本」只读到开头；首页导航 / 分类 / 书库入口调整（2026-09-25）

**现象**（用户报）：「【忘尘山:高冷仙子皆为炉鼎】1-23」这类书只有一个章节，点进去只有第 1-2 章，
3~23 章看不到 —— "并不会继续加载剩余的页面"。同一批诉求还有：顶部导航只留「书库」、分类要换行、
「按书源浏览」挪到书源搜索页、整个书库要有「查看全部」。

**根因（只有第一条是 bug）**：禁忌书屋（cool18）这类论坛书源的 `ruleToc` 是「一个帖子 = 一本单章书」，
`chapterUrl` 就是 `@js:baseUrl`；合集帖的**帖子正文里**用 `<a>` 列出其余分帖（3、4-6 … 22-23）。
`fetch_book` 只看到 TOC 规则返回的那一个自身条目，11 个分帖从未被抓取 —— 内容不是被截断，
而是从来没下过（线上核对：`chapters` 只有 1 行，`000001.md` 只含第 1-2 章正文）。
书源自带的 `bookSourceComment` 明写「不自动合并同系列多帖」，所以这不是源站 bug，是 NovelHub
缺一层「帖子正文里的分帖链接 = 本书的后续章节」的识别。

**改动**：

- `yuedu/parsing.py`：新增 `_content_body_elements()`（用**书源自己的 `ruleContent.content` 选择器**
  定位正文元素）、`_series_index_parts()`（正文内的同站链接 → 分帖）与 `_title_key` /
  `_common_key_prefix` / `_key_prefix_tail` 三个纯函数。判据四条**同时**成立：在正文元素内、同主机、
  不是书页自身 URL；候选 ≥2 条；链接标题的公共前缀 ≥4 字且是本书标题的前缀；每条去掉公共前缀后的
  尾部很短（≤12 字）且含数字（`（3）`/`（4-6）`）。分帖按**声明的分节号**排序（帖子把最新的列在最前）。
- `yuedu/book.py`：`fetch_book` 只在「TOC 只解析出书页自身这一个章节」的那一支里调用检测，
  命中就把分帖接成本书第 2..N 章（书页自身保持第 1 章，它载着开篇）。
- `yuedu/selectors.py`：新增 `SERIES_INDEX_*` 词表（导航锚文本、尾标记长度上限、前缀下限、条数上限）。
- 前端：`NavBar.vue` 顶部只留「书架 / 书库」；`BooksPage.vue` 分类栏改 `flex-wrap`（不再横向滚动）、
  新增「全部书籍」入口（`?all=1`，分页列出整个书库）、「最新入库」标题右侧补「查看全部」；
  「按书源浏览」从书架页与书库首页移到 **书库 → 书源搜索** 标签页下（原先两处各有一份，已收敛为一处）。
- `stores/i18n.ts`：新增 `books_all_books`；删掉不再使用的 `nav_novels`/`nav_comics`；补
  `reader_bookmark_add`/`reader_bookmark_remove` —— 书签按钮的 `:title` 一直在用这两个键，
  但两个语言表里都没有，悬停显示的是键名本身（第 6 项「历史遗留」排查时发现）。

**验证**：新增 4 条测试（合集帖展开并按分节号排序、正文外的「相关推荐」不展开、正文里的无关链接
不展开、「返回主帖 / 下一页」这类导航锚点不展开）；全量 `914 passed`；把线上真实 HTML
（`tid=14536503`）与线上书源 JSON 喂给 `fetch_book` 复跑 → 12 章（1-2 / 3 / 4-6 / 7 / 8-10 / 11-12 /
13 / 14-16 / 17 / 18-19 / 20-21 / 22-23），普通单帖仍是 1 章。

**要用户做的事**：镜像重建后，对这类合集帖点一次「重新同步」。章节按 URL 去重，已下载的第 1 章
不会重下，只补 2..N 章；单帖书（该源 12091 本）不受影响。
