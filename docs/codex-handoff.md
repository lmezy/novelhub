...
## 17. 2026-09-11 补充：更新镜像后“后台都是报错”的复核

用户重建镜像（09-11 00:12）后同步 7 个书源，反馈“后台都是报错”。逐项复核结果：真正的
代码缺陷有两处（禁忌书屋误判、瞬时失败被当成终止失败），其余是站点侧 Cloudflare 防护。

### 复核到的现象（线上）

- 要撸小说：00:17 的任务 48 秒内失败，错误 `书源未返回可同步的书籍...`；同一时间日志里
  每个分类都是 `Explore kind failed: 都市 ()`（**空错误信息**）或浏览器
  `Page.goto: net::ERR_CONNECTION_CLOSED`。**而 00:19 重新发起的任务一切正常**
  （当前进度 28 章已入库、0 失败）；同一容器内单独跑 `discover_books` 也稳定返回 10~80 本。
  => 是**镜像更新后那几分钟**代理/网络瞬时不可用，任务却被记为“终止失败”，
  且错误信息误导成“书源没返回书”。
- 禁忌书屋 cool18：列表页正常（28940 字节、无拦截标记），但帖子页被判为
  `anti-bot/captcha` → 任务失败。抓原始 HTML 后发现页面完全正常
  （标题 `【云雨箱庭？大胆去干！】（34） 作者：Xiaodie Zhuang`），**只是页面 JS 里有一句
  `alert('举报失败，请稍后再试')`**，命中了 `STRONG_BLOCK_MARKERS` 里的“请稍后再试”
  => **误判**。
- 第一版主 banzhu44444444.net：抓原始页确认 `title='Just a moment...'` +
  `challenge-platform` + “正在进行安全验证” → 真 Cloudflare 挑战（同 SiS、御宅屋），
  需 Cookie/换节点，代码不绕过。
- 爱丽丝书屋：27 本书已入库、章节持续创建（上一轮的并发/限速加固生效）。

### 本轮代码修复

`backend/app/crawler/plugins/yuedu/__init__.py`

- **拦截误判修复**：把“请稍后再试 / 请稍后重试 / 请完成验证 / 安全验证 / 身份验证”从
  无条件 `STRONG_BLOCK_MARKERS` 改为**上下文判定**（`has_contextual_block_marker()`：
  仅在前后 90 字符内出现“验证码/人机/频繁/限流/访问异常/自动程序/安全服务/captcha/
  challenge”等才判定为拦截）。cool18 的 `alert('举报失败，请稍后再试')` 不再误伤；
  真正的限流/验证页（`访问异常，请稍后再试`、`请求过于频繁，请稍后再试`、GoEdge、
  Cloudflare）仍被识别。`_content_is_blocked()` 同步采用该规则，避免正文章节里出现
  “请稍后再试”这类台词被整章丢弃。
- **发现阶段区分“网络故障”与“没有书”**：`fetch_explore` 新增 transport 错误收集，
  无结果且全部是网络/代理失败时抛出
  `书源目录暂时无法访问（网络/代理错误，请稍后重试）：ConnectTimeout`，
  不再报成误导性的“书源未返回可同步的书籍”；分类失败日志也带上异常类型
  （原来空错误信息只显示 `()`）。

`backend/app/services/crawl_runner.py`

- **任务级自动重试**：`_is_transient_task_error()` 识别超时/连接/代理类错误（含
  `httpx.*Timeout/ConnectError/ReadError` 这类 `str()` 为空的异常），
  命中则把任务改回 `pending` 并设置 `resume_at = now + 60/120s`
  （`SYNC_TASK_MAX_AUTO_RETRIES` 默认 2，`SYNC_TASK_RETRY_BASE_SECONDS` 默认 60），
  同时把 `error` 写成“网络/代理暂时不可用（ConnectTimeout），60 秒后自动重试（1/2）”。
  anti-bot/验证码/书源规则类错误**不**重试，避免反复打站。

### 验证

- 后端全量测试：**389 passed**（新增上下文拦截判定、cool18 误判回归、
  transport 错误上报、任务瞬时失败重试 / 验证码不重试等用例）。
- 真实站点回归（容器内影子加载改动文件，不动线上代码）：
  - cool18 帖子页：`blocked=False`、`title='【云雨箱庭？大胆去干！】（34）'`、
    1 章、正文 **13567 字**（修复前该任务直接失败）
  - 第一版主：确认 `title='Just a moment...'`，属真 Cloudflare
- 线上并行任务复核：要撸小说（28 章 / 0 失败）、爱丽丝书屋（27 本）均正常入库。

### 用户需要做的

1. 重建并重启 `crawler`（+`backend`）后，禁忌书屋与 h528 可正常同步。
