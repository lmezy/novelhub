...
- 顶部导航只有 **书架** 与 **书库**：**书库**（`/books`）是包含小说和漫画的全部列表，
  页头有「全部 / 小说 / 漫画」切换（`/novels`、`/comics` 仍是这两种视图的地址）；
- 书库首页顶部是分类栏（可以换行显示所有分类）与「**全部书籍**」，后者分页列出整个书库，
  「最新入库」和每个分类标题右侧的「查看全部」也是同一个入口；
- 「按书源浏览」在 **书库 → 书源搜索** 标签页下面，每个书源一块，点「查看全部」即按书源浏览；
